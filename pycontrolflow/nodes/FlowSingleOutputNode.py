...
class FlowSingleOutputNode(FlowNode, IFlowValueProvider[TFlowSingleOutputNodeType],
                           typing.Generic[TFlowSingleOutputNodeType], ABC):
    ...
    def to(self,
           output_value: FlowValue[TFlowSingleOutputNodeType]) -> 'FlowSingleOutputNode[TFlowSingleOutputNodeType]':
        node_type = self._get_output_type()
        output_value_type = output_value.get_type()

        if output_value_type != node_type:
            raise TypeError(
                f"output type does not match expected type, got: {output_value_type}, expected: {node_type}")
